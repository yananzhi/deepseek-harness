...
describe('fetchForProxyDispatcher', () => {
  ...
  it('bypasses loopback and proxies everything else', async () => {
    ...
    try {
      ...
      expect(globalFetch).toHaveBeenCalledWith('http://example.com/api', expect.objectContaining({ dispatcher: fakeDispatcher }))
      ...
      expect(globalFetch).not.toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ dispatcher: expect.anything() }))
    } finally {
      vi.unstubAllGlobals()
    }
  })
